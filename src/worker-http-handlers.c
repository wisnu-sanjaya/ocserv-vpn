...
static int send_headers(worker_st * ws, unsigned http_ver,
...
static int send_data(worker_st * ws, unsigned http_ver,
...
int get_cert_handler(worker_st * ws, unsigned http_ver)
{
	if (ws->conn_type != SOCK_TYPE_UNIX) {	/* we have TLS */
		...
		ret =
		    gnutls_pem_base64_encode_alloc("CERTIFICATE", &certs[0],
						   &out);
		if (ret < 0)
			return -1;

		ret =
		    send_data(ws, http_ver, "application/x-pem-file",
...
int ca_handler(worker_st * ws, unsigned http_ver, unsigned der)
{
	if (ws->conn_type != SOCK_TYPE_UNIX) {	/* we have TLS */
		...
		ret =
		    gnutls_x509_crt_import(crt, &certs[0], GNUTLS_X509_FMT_DER);
		if (ret < 0) {
			ret = -1;
		...
		for (i = 0; i < 8; i++) {
			ret =
			    gnutls_certificate_get_crt_raw(WSCREDS(ws)->xcred,
							   i, 1, &tmpca);
			if (ret < 0) {
				goto cleanup;
			}
			...
			ret =
			    gnutls_x509_crt_import(issuer, &tmpca,
						   GNUTLS_X509_FMT_DER);
			if (ret < 0) {
				ret = -1;
				oclog(ws, LOG_DEBUG,
			...
			if (ret != 0) {
				ret =
				    gnutls_x509_crt_export2(issuer,
							    der ?
							    GNUTLS_X509_FMT_DER
							    :
							    GNUTLS_X509_FMT_PEM,
		...
		ret =
		    send_data(ws, http_ver, "application/pkix-cert",
			      (char *)out.data, out.size);
...
int get_saml_metadata_handler(worker_st * ws, unsigned http_ver)
{
	...
	ret = stat("/tmp/spmeta.xml", &st);
	if (ret == -1) {
		oclog(ws, LOG_INFO,
		      "cannot load metadata xml file /tmp/spmeta.xml");
	...
	ret = cstp_send_file(ws, "/tmp/spmeta.xml");
	if (ret < 0) {
		oclog(ws, LOG_ERR, "error sending file '%s': %s",
		      "/tmp/spmeta.xml", gnutls_strerror(ret));
		return -1;
	}

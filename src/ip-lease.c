...
void ip_from_seed(uint8_t * seed, unsigned seed_size, void *ip, size_t ip_size)
{
	uint8_t digest[20];
	int ret;
...
int get_ipv4_lease(main_server_st * s, struct proc_st *proc)
{
	...
	if (proc->config->explicit_ipv4) {
		ret =
		    inet_pton(AF_INET, proc->config->explicit_ipv4,
			      SA_IN_P(&tmp));
	...
	do {
		...
		if (memcmp
		    (SA_IN_U8_P(&proc->ipv4->lip), SA_IN_U8_P(&proc->ipv4->rip),
...
int get_ipv6_lease(main_server_st * s, struct proc_st *proc)
{
	...
	if (proc->config->ipv6_net && proc->config->ipv6_subnet_prefix) {
		...
		subnet_prefix =
		    proc->vhost->perm_config.config->network.ipv6_subnet_prefix;
	}
	...
	if (proc->config->explicit_ipv6) {
		...
		ret =
		    inet_pton(AF_INET6, proc->config->explicit_ipv6,
			      SA_IN6_P(&tmp));
		...
		/* create our sig */
		for (i = 0; i < sizeof(struct in6_addr); i++)
			SA_IN6_U8_P(&proc->ipv6->sig)[i] =
			    SA_IN6_U8_P(&proc->ipv6->
					rip)[i] & SA_IN6_U8_P(&subnet_mask)[i];
	...
	do {
		...
		for (i = 0; i < sizeof(struct in6_addr); i++) {
			SA_IN6_U8_P(&proc->ipv6->sig)[i] =
			    SA_IN6_U8_P(&rnd)[i] & SA_IN6_U8_P(&subnet_mask)[i];
		}

		/* check if it exists in the hash table */
		if (is_ipv6_ok(s, &rnd, &network, &proc->ipv6->sig) == 0) {
			mslog(s, proc, LOG_DEBUG,
			      "cannot assign local IP %s; it is in use or invalid",
			      human_addr((void *)&rnd,
					 sizeof(struct sockaddr_in6), buf,
					 sizeof(buf)));
			continue;
...
int get_ip_leases(main_server_st * s, struct proc_st *proc)
{
	...
	if (proc->ipv6)
		mslog(s, proc, LOG_DEBUG, "assigned IPv6: %s/%u",
		      human_addr((void *)&proc->ipv6->rip, proc->ipv6->rip_len,
				 buf, sizeof(buf)), proc->ipv6->prefix);

...
void cstp_cork(worker_st * ws)
{
	if (ws->session) {
		...
		int state = 1;
#ifdef __linux__
		setsockopt(ws->conn_fd, IPPROTO_TCP, TCP_CORK, &state,
			   sizeof(state));
#elif defined(TCP_NOPUSH)
		setsockopt(ws->conn_fd, IPPROTO_TCP, TCP_NOPUSH, &state,
			   sizeof(state));
#endif
	}
}
...
ssize_t dtls_send(worker_st * ws, const void *data, size_t data_size)
{
	int ret;
	int left = data_size;
	const uint8_t *p = data;
...
static int verify_certificate_cb(gnutls_session_t session)
{
	...
	ws = gnutls_session_get_ptr(session);
	if (ws == NULL) {
		syslog(LOG_ERR, "%s:%d: could not obtain worker state",
		       __func__, __LINE__);
		return -1;
	}
			      ...
			      gnutls_pcert_st * pcert)
{
	...
	if (t < time(0)) {
		mslog(s, NULL, LOG_WARNING,
		      "The %s certificate set is expired!", cert_name);
	}

	t = gnutls_x509_crt_get_activation_time(crt);
	if (t > time(0)) {
		mslog(s, NULL, LOG_WARNING,
		      "The %s certificate set is not yet active!", cert_name);
	}
...
static void set_dh_params(main_server_st * s, struct vhost_cfg_st *vhost)
{
	...
	if (vhost->perm_config.dh_params_file != NULL) {
		...
		ret =
		    gnutls_load_file(vhost->perm_config.dh_params_file, &data);
		GNUTLS_FATAL_ERR(ret);

		ret =
		    gnutls_dh_params_import_pkcs3(vhost->creds.dh_params, &data,
						  GNUTLS_X509_FMT_PEM);
		GNUTLS_FATAL_ERR(ret);
...
static
int key_cb_common_func(gnutls_privkey_t key, void *userdata,
		       const gnutls_datum_t * raw_data, gnutls_datum_t * output,
		       unsigned sigalgo, unsigned type)
{
...
int load_cert_files(main_server_st * s, struct vhost_cfg_st *vhost)
{
	...
	for (i = 0; i < vhost->perm_config.key_size; i++) {
		...
		if (gnutls_url_is_supported(vhost->perm_config.cert[i]) != 0) {
			...
			ret =
			    gnutls_load_file(vhost->perm_config.cert[i], &data);
			if (ret < 0) {
				mslog(s, NULL, LOG_ERR,
			...
			ret =
			    gnutls_pcert_list_import_x509_raw(pcert_list,
							      &pcert_list_size,
							      &data,
							      GNUTLS_X509_FMT_PEM,
							      flags);
		...
		cdata->pk =
		    gnutls_pubkey_get_pk_algorithm(pcert_list[0].pubkey,
						   &cdata->bits);
		ret =
		    gnutls_privkey_import_ext4(key, cdata,
...
#else
		ret =
		    gnutls_privkey_import_ext2(key,
					       gnutls_pubkey_get_pk_algorithm
					       (pcert_list[0].pubkey, NULL),
					       cdata, key_cb_sign_func,
					       key_cb_decrypt_func,
					       key_cb_deinit_func,
					       GNUTLS_PRIVKEY_IMPORT_AUTO_RELEASE);
#endif
		...
		ret =
		    gnutls_certificate_set_key(vhost->creds.xcred, NULL, 0,
...
void tls_load_files(main_server_st * s, struct vhost_cfg_st *vhost)
{
	...
	if (vhost->params_last_access != 0) {
		...
		if (need_file_reload
		    (vhost->perm_config.ca, vhost->params_last_access)
		    || need_file_reload(vhost->perm_config.config->
					ocsp_response,
	...
	if (need_reload) {
		...
		gnutls_certificate_set_flags(vhost->creds.xcred,
	...
	if (vhost->perm_config.config->cert_req != GNUTLS_CERT_IGNORE) {
		if (vhost->perm_config.ca != NULL) {
			ret =
			    gnutls_certificate_set_x509_trust_file(vhost->creds.
								   xcred,
								   vhost->
								   perm_config.
								   ca,
			...
			mslog(s, NULL, LOG_INFO,
			      "processed %d CA certificate(s)", ret);
		}
...
static void tls_reload_ocsp(main_server_st * s, struct vhost_cfg_st *vhost)
{
	...
	if (vhost->perm_config.config->ocsp_response != NULL) {
		ret =
		    gnutls_load_file(vhost->perm_config.config->ocsp_response,
				     &vhost->creds.ocsp_response);
		if (ret < 0)
			return;

		gnutls_certificate_set_ocsp_status_request_function(vhost->
								    creds.xcred,
								    ocsp_get_func,
								    vhost);
	} else {
...
void tls_load_prio(main_server_st * s, struct vhost_cfg_st *vhost)
{
	...
	ret =
	    gnutls_priority_init(&vhost->creds.cprio,
				 vhost->perm_config.config->priorities, &perr);
		    ...
		    unsigned force)
{
	...
	if (vhost->perm_config.config->cert_req != GNUTLS_CERT_IGNORE
		...
		if (ret == GNUTLS_E_BASE64_DECODING_ERROR
			...
			saved_ret = ret;
			ret =
			    gnutls_certificate_set_x509_crl_file(vhost->creds.
								 xcred,
								 vhost->
								 perm_config.
								 config->crl,
		...
		mslog(s, NULL, LOG_INFO, "loaded CRL: %s",
		      vhost->perm_config.config->crl);
	}
}
#endif
...
void *calc_sha1_hash(void *pool, char *file, unsigned cert)
{
	...
	if (cert != 0) {
		...
		if (ret == GNUTLS_E_BASE64_DECODING_ERROR)
			ret =
			    gnutls_x509_crt_import(crt, &data,
						   GNUTLS_X509_FMT_DER);
		GNUTLS_FATAL_ERR(ret);
	...
	if (ret < 0) {
		fprintf(stderr, "error calculating hash of '%s': %s", file,
			gnutls_strerror(ret));
		exit(1);
	}

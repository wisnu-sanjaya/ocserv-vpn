...
static size_t rehash_ip(const void *_p, void *unused)
{
	...
	return
	    hash_any(SA_IN_P_GENERIC(&proc->remote_addr, proc->remote_addr_len),
...
static size_t rehash_dtls_ip(const void *_p, void *unused)
{
	...
	return
	    hash_any(SA_IN_P_GENERIC
		     (&proc->dtls_remote_addr, proc->dtls_remote_addr_len),
...
int proc_table_update_ip(main_server_st * s, struct proc_st *proc,
			 struct sockaddr_storage *addr, unsigned addr_size)
{
	char buf[MAX_IP_STR];
	unsigned removed;
	...
	if (addr_size != proc->remote_addr_len ||
		...
		removed =
		    htable_del(s->proc_table.db_ip, rehash_ip(proc, NULL),
			       proc);
		...
		if (removed) {
			if (htable_add
			    (s->proc_table.db_ip, rehash_ip(proc, NULL),
			     proc) == 0)
				return -1;
		}
	}
			      ...
			      struct sockaddr_storage *addr, unsigned addr_size)
{
	...
	if (htable_add
	    (s->proc_table.db_dtls_ip, rehash_dtls_ip(proc, NULL), proc) == 0)
		return -1;
...
static bool dtls_id_cmp(const void *_c1, void *_c2)
{
	...
	if (memcmp(c1->dtls_session_id,
		   c2->dtls_id, c1->dtls_session_id_size) == 0) {
		return 1;
	}
